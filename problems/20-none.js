...
let none = function(arr, cb) {
    // filter the array to only include those that are false when applied function.
    // return if the new array has the same amount of element with the original array
    let result = arr.filter((el) => !(cb(el)))
